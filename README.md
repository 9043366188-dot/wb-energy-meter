...
С версии 0.9.0 в «Настройках» есть блок «Обновление»: сервис сам качает
свежий код из GitHub, устанавливает и перезапускается.

- **Источник — ветка `main`, не теги.** Что в `main` на GitHub на момент
  нажатия кнопки, то и приезжает на контроллер.
- **Автопроверки нет.** Сервис не ходит в интернет, пока вы не нажали
  «Проверить обновления».
- Нажали «Обновить сейчас» → подтвердили в диалоге → сервис остановится
  примерно на минуту (сбор данных прервётся), установится новая версия,
  сервис перезапустится. База данных сохраняется, `install.sh` делает
  её резервную копию, как и при обычной установке.
- **Автоматический откат.** Если после установки сервис не поднимается
  в течение 90 секунд — автоматически откатывается на предыдущую
  версию из резервной копии. Если откат тоже не помог — статус
  «Провал» с инструкцией зайти по SSH
  (`journalctl -u wb-energy-meter -n 100`).
- Прогресс виден прямо в браузере: во время перезапуска запросы к API
  ненадолго начнут падать — это нормально, страница продолжает
  опрашивать статус и не считает это ошибкой первые 120 секунд.

> **Внимание, у сервиса нет аутентификации.** Если `http.host: 0.0.0.0`
> (по умолчанию) и контроллер в сети, доступной кому-то ещё —
> **любой в этой сети может нажать кнопку «Обновить сейчас» и
> инициировать перезапуск сервиса учёта электроэнергии.** Если
> контроллер в недоверенной сети, сделайте одно из двух:
> - поставьте `http.host: 127.0.0.1` в конфиге и ходите в веб-интерфейс
>   через SSH-туннень (`ssh -L 8080:127.0.0.1:8080 root@<IP>`);
> - или `update.allow_from_ui: false` — кнопка скроется, обновление
>   останется доступно только вручную:
>   `sudo bash scripts/install-from-github.sh` по SSH.
>
> Своя система паролей/токенов в рамках этой функциональности
> сознательно не реализована — это отдельная задача.

Настройки — секция `update:` в `/etc/wb-energy-meter.conf`, см.
[Конфигурацию](#конфигурация) ниже.
...
| GET | `/api/update/check` | Сверить установленную версию с веткой `main` на GitHub. 503 при `update.enabled: false` |
| GET | `/api/update/status` | Статус текущего/последнего обновления. Без файла статуса — `{"state":"idle"}`, никогда не 500 |
| POST | `/api/update/start` | Запустить обновление. Body: `{"commit":"<sha из /api/update/check>"}`. 403 при `allow_from_ui: false`, 409 если уже идёт или sha устарел |
...
# Самообновление из GitHub по кнопке (v0.9.0+). Старые конфиги без этой
# секции продолжают работать — значения по умолчанию заданы в коде.
update:
  enabled: true
  allow_from_ui: true    # см. предупреждение в разделе "Обновление" выше
...
| 9 | 0.9.0 | Самообновление из GitHub по кнопке, автооткат | ✅ |
| 10 | 0.10.0 | Двух-тарифный учёт (день/ночь) | ⏳ |
| 11 | 0.11.0 | Алерты + SMTP уведомления | ⏳ |
| 12 | 0.12.0 | Excel-отчёты + автоматическая рассылка | ⏳ |
| 13 | 1.0.0 | .deb-пакет, финальная документация | ⏳ |
